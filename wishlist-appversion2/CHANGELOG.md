...
## 2026-08-17 - 리뷰는 표정만으로 올리고, 탈퇴 시 소셜 흔적은 익명화

🌟 **쉬운 설명**
- 리뷰는 제목·본문 없이 표정만 골라도 올릴 수 있습니다.
- 탈퇴하면 위시리스트와 리뷰는 지워지고, 친구가 받은 살까말까와 댓글은 탈퇴한 사용자로 남습니다.

🔧 **기술 설명**
- `publishReview`의 제목·본문 필수 검사를 제거하고, 피드/상세는 빈 텍스트를 숨긴다.
- 계정 삭제 전에 보낸 살까말까 사본과 댓글 작성자 표시를 `탈퇴한 사용자`로 바꾼 뒤, 리뷰·공유 HTML·아바타 Storage를 지운다.
- `receivedBaskets`는 보낸 사람만 작성자 필드(`ownerName`, `fromHandle`, `fromAvatar`, `title`)를 익명화하도록 규칙을 연다.
