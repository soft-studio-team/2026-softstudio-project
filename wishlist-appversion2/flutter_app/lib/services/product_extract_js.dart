...
(function () {
  ...
  function extractVerifiedSitePricing(){
    ...
    if(hostIs('oliveyoung.co.kr')){
      // 정가=options[].salePrice. 쿠폰/프로모(maxBenefit)로 낮추지 않는다.
      var ostates={};document.querySelectorAll('script').forEach(function(x){var txt=x.textContent||'',m=/^\s*self\.__next_f\.push\((\[.*\])\)\s*$/s.exec(txt);if(!m)return;var payload;try{payload=JSON.parse(m[1]);}catch(e){return;}if(!Array.isArray(payload)||typeof payload[1]!=='string')return;var dec=payload[1],rx=/"data"\s*:\s*\{\s*"data"\s*:\s*\{/g,dm;while((dm=rx.exec(dec))){var obj=objectAt(dec,dm.index+dm[0].length-1),cand;try{cand=JSON.parse(obj);}catch(e){continue;}var gn=String(cand&&cand.goodsNumber||'');if(!gn||!Array.isArray(cand.options))continue;function oscore(g){return (g&&g.options?g.options.length:0)+(g&&g.saleableFlag===true?10:0);}if(!ostates[gn]||oscore(cand)>oscore(ostates[gn]))ostates[gn]=cand;}});var owant=(/goodsNo=([A-Z0-9]+)/i.exec(location.href)||[])[1];if(!owant||!ostates[owant])return null;state=ostates[owant];if(state.saleableFlag!==true||state.displayableFlag!==true||state.soldOutFlag===true||String(state.status||'')!=='20'||!Array.isArray(state.options))return null;var ol=state.options.filter(function(o){return o&&o.soldOutFlag!==true;});if(!ol.length)return null;var sales=uniqueRows(ol.map(function(o){return toPrice(o.salePrice);}).filter(Boolean));if(!sales.length)return null;var olo=Math.min.apply(null,sales),ohi=Math.max.apply(null,sales),odep=olo!==ohi;var oyname=firstStr(state.goodsName),oyt=pageText();if(!oyname||oyt.indexOf(oyname)<0||(oyt.indexOf('장바구니')<0&&oyt.indexOf('바로구매')<0&&oyt.indexOf('구매하기')<0))return null;
      return result('oliveyoung',olo,null,'goods.options[].salePrice',null,{priceConfidence:'medium',purchasePriceStatus:odep?'option_dependent':'confirmed',optionDependent:odep,optionPriceMin:odep?olo:null,optionPriceMax:odep?ohi:null});
    }
