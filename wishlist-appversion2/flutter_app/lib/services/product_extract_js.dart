...
(function () {
  ...
  function extractVerifiedSitePricing(){
    ...
    if(hostIs('aritzia.com')){
      // variant.price는 CAD/USD 소수일 수 있어 KRW UI(₩…)를 우선하고, 1000 미만은 통화 오인으로 버린다.
      var mob=document.querySelectorAll('#mobify-data');if(mob.length!==1)return null;try{var ar=JSON.parse(mob[0].textContent),pre=ar.__PRELOADED_STATE__,pp=pre.pageProps.structuredDataProps,seo=pp.seoProduct,store=pre.__STATE_MANAGEMENT_LIBRARY.store.productStore,by=store.productsById;}catch(e){return null;}var pid=String(seo&&seo.id||''),an=firstStr(seo&&seo.displayName);if(!/^\d+$/.test(pid)||!an||!by||!by[pid])return null;var colors=[];asArray(pp&&pp.structuredData).forEach(function(p){if(exactType(p,'Product')&&String(p.sku||'')===pid){var m=/[?&]color=(\d+)/.exec(String(p['@id']||p.url||''));if(m)colors.push(m[1]);}});var cm=/[?&]color=(\d+)/.exec(location.href||'');if(cm)colors.push(cm[1]);colors=uniqueRows(colors);if(colors.length!==1)return null;var av=[];asArray(by[pid].variants).forEach(function(v){if(v&&v.variationValues&&String(v.variationValues.color||'')===colors[0]&&v.orderable===true){var p=toPrice(v.price);if(p&&p>=1000)av.push(p);}});av=uniqueRows(av);var box=document.querySelectorAll('[data-testid="product-price-text"]');function localized(root,sel){if(!root)return null;var es=root.querySelectorAll(sel);if(es.length!==1)return null;var t=(es[0].textContent||'').trim().replace(/\u00a0/g,' '),m=/₩\s*([\d,]+)/.exec(t),p=m?toPrice(m[1]):null;return p&&p>=1000?p:null;}var root=box.length===1?box[0]:null;regular=localized(root,'[data-testid="product-list-price-text"]');var se=root?root.querySelectorAll('[data-testid="product-list-sale-text"]'):[];if(se.length>1)return null;sale=se.length?localized(root,'[data-testid="product-list-sale-text"]'):regular;if(!sale&&av.length===1)sale=av[0];if(!sale||sale<1000)return null;if(av.length===1&&sale!==av[0])return null;var areg=se.length&&regular&&regular>sale?regular:null;var at=pageText();if(at.indexOf(an)<0)return null;if(!(/Add to Bag|Bag|장바구니/.test(at)))return null;if(!(at.indexOf('₩'+Number(sale).toLocaleString('en-US'))>=0||at.indexOf(won(sale))>=0||at.indexOf(Number(sale).toLocaleString('en-US'))>=0))return null;
      return result('aritzia',sale,areg,'localized KRW list/sale UI (+ Mobify KRW variant cross-check)',areg?'product-list-price-text':null,{optionDependent:false,priceConfidence:'medium'});
    }
    ...
    if(hostIs('oliveyoung.co.kr')){
      var ostates={},obad={};document.querySelectorAll('script').forEach(function(x){var txt=x.textContent||'',m=/^\s*self\.__next_f\.push\((\[.*\])\)\s*$/s.exec(txt);if(!m)return;var payload;try{payload=JSON.parse(m[1]);}catch(e){return;}if(!Array.isArray(payload)||typeof payload[1]!=='string')return;var dec=payload[1],rx=/"data"\s*:\s*\{\s*"data"\s*:\s*\{/g,dm;while((dm=rx.exec(dec))){var obj=objectAt(dec,dm.index+dm[0].length-1),cand;try{cand=JSON.parse(obj);}catch(e){continue;}var gn=String(cand&&cand.goodsNumber||'');if(!gn)continue;if(ostates[gn]&&JSON.stringify(ostates[gn])!==JSON.stringify(cand)){obad[gn]=true;continue;}ostates[gn]=cand;}});var owant=(/goodsNo=([A-Z0-9]+)/i.exec(location.href)||[])[1];if(!owant||!ostates[owant]||obad[owant])return null;state=ostates[owant];if(state.saleableFlag!==true||state.displayableFlag!==true||state.soldOutFlag===true||String(state.status||'')!=='20'||!Array.isArray(state.options))return null;var ol=state.options.filter(function(o){return o&&o.soldOutFlag!==true;});if(!ol.length)return null;var sales=uniqueRows(ol.map(function(o){return toPrice(o.salePrice);}).filter(Boolean));if(!sales.length)return null;var olo=null,ohi=null,odep=false,oreg=null,osrc='goods.options[].salePrice',osrcReg=null,checkPrices=[],ob=state.maxBenefitPriceDto,opp=[],promoOk=false;if(ob){var rep=ol.find(function(o){return String(o.optionNumber||'')===String(ob.optionNumber||'');}),promo=toPrice(ob.promotionSalePrice),orig=toPrice(ob.originalSalePrice),finalp=toPrice(ob.finalPrice);if(rep&&promo&&orig&&promo<=orig&&toPrice(rep.salePrice)===orig){promoOk=true;for(i=0;i<ol.length;i++){var os=toPrice(ol[i].salePrice);if(!os){promoOk=false;break;}if(String(ol[i].optionNumber||'')===String(ob.optionNumber||'')||os===orig)opp.push(promo);else if(ob.hasCoupon!==true){var of=toPrice(ol[i].finalPrice);if(!of||of>os){promoOk=false;break;}opp.push(of);}else{promoOk=false;break;}}}if(promoOk&&opp.length){olo=Math.min.apply(null,opp);ohi=Math.max.apply(null,opp);odep=olo!==ohi;var oregs=uniqueRows(ol.map(function(o){return toPrice(o.salePrice);}).filter(Boolean));oreg=oregs.length===1&&oregs[0]>olo&&oregs[0]>=ohi?oregs[0]:null;osrc='goods.maxBenefitPriceDto.promotionSalePrice + goods.options[soldOutFlag=false]';osrcReg=oreg?'goods.options[].salePrice':null;checkPrices=[finalp,orig,olo,ohi,promo];}else if(promo&&orig&&promo<=orig&&rep&&toPrice(rep.salePrice)===orig){olo=ohi=promo;odep=false;oreg=orig>promo?orig:null;osrc='goods.maxBenefitPriceDto.promotionSalePrice (rep option)';osrcReg=oreg?'goods.maxBenefitPriceDto.originalSalePrice':null;checkPrices=[promo,orig,finalp];}}if(olo==null){if(sales.length!==1)return null;olo=ohi=sales[0];odep=false;oreg=null;osrc='goods.options[].salePrice';osrcReg=null;checkPrices=[olo];if(ob){var ox=toPrice(ob.originalSalePrice),op=toPrice(ob.promotionSalePrice);if(ox)checkPrices.push(ox);if(op)checkPrices.push(op);}}var oyname=firstStr(state.goodsName),oyt=pageText();if(!oyname||oyt.indexOf(oyname)<0||(oyt.indexOf('장바구니')<0&&oyt.indexOf('바로구매')<0&&oyt.indexOf('구매하기')<0))return null;var oseen=false;for(i=0;i<checkPrices.length;i++){if(!checkPrices[i])continue;if(oyt.indexOf(won(checkPrices[i]))>=0||oyt.indexOf(Number(checkPrices[i]).toLocaleString('en-US'))>=0){oseen=true;break;}}if(!oseen)return null;
      return result('oliveyoung',olo,oreg,osrc,osrcReg,{priceConfidence:'medium',purchasePriceStatus:odep?'option_dependent':'confirmed',optionDependent:odep,optionPriceMin:odep?olo:null,optionPriceMax:odep?ohi:null});
    }
