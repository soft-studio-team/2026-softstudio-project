...
다음: §0.13 패치 우선순위. **1~3차** 완료. **이미지/패션플러스** MATCH. **회원가 베이스라인:** 어반스터프·파브레가=live×0.9, 필루미네이트 엔진<live, 리·비바 `price_ambiguous` — 정책 결정 전 엔진 변경 금지. 남음: 에이블리 로드, 앤더슨벨#2, SSG 단품 카탈로그, 더현대Hi·4910 live 가격, 회원가 정책.
